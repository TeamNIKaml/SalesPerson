...
import com.energyeye.salesperson.properties.YieldMethodDataSource;

public class YieldMethodUnitTest extends AndroidTestCase {

    private static final String TEST_FILE_PREFIX = "test_";
    private DBHelper helper;
    private SQLiteDatabase db;
    private YieldMethodDataSource dataSource = YieldMethodDataSource
	    .getYieldMethodDataSource();
	...
	helper = new DBHelper(context, 1, Constants.DB_NAME,
		Constants.YIELD_METHOD_CREATE_QUERY);
	...
	db = helper.getWritableDatabase();
	 dataSource.setName("Nikhil");
	    db.insert(Constants.YIELD_METHOD_TABLE_NAME, null,
		    dataSource.yieldMethodToContentValues());
	  db.close();
    }
    ...
    private void selectAllSmall()
    {
	  db = helper.getReadableDatabase();
			Cursor c = db.query(Constants.YIELD_METHOD_TABLE_NAME, null,
	...
	db = helper.getWritableDatabase();
	db.delete(Constants.YIELD_METHOD_TABLE_NAME, null, null);
	db.close();
    }
	...
	db.update(Constants.YIELD_METHOD_TABLE_NAME,
		dataSource.yieldMethodToContentValues(), "_id = ?",
		new String[] { "1" });
	...
	System.out.println("################### Insert Medium ##########################3");
	db = helper.getWritableDatabase();
	for (int i = 0; i < 10; i++) {
	...
	System.out.println("#########################@ Medium update ####################3");
	db = helper.getWritableDatabase();	
	...
	db = helper.getWritableDatabase();
	db.delete(Constants.YIELD_METHOD_TABLE_NAME, "_id = ?", new String[] { "7" });
	db.close();
	...
	System.out.println("#################################Insert Large#####################");
	db = helper.getWritableDatabase();
	for (int i = 0; i < 100; i++) {
	    dataSource.setName("Nikhil" + String.valueOf(i));
	    db.insert(Constants.YIELD_METHOD_TABLE_NAME, null,
	...
	db = helper.getWritableDatabase();
	db.delete(Constants.YIELD_METHOD_TABLE_NAME, "_id = ?", new String[] { "34" });
	db.close();
