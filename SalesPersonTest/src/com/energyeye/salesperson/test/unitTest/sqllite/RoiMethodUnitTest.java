...
import com.energyeye.salesperson.properties.RoiMethodDataSource;

public class RoiMethodUnitTest extends AndroidTestCase {

    private static final String TEST_FILE_PREFIX = "test_";
    private DBHelper helper;
    private SQLiteDatabase db;
    private RoiMethodDataSource dataSource = RoiMethodDataSource
	    .getRoiMethodDataSource();
	...
	helper = new DBHelper(context, 1, Constants.DB_NAME,
		Constants.ROI_METHOD_CREATE_QUERY);
	...
	db = helper.getWritableDatabase();
	 dataSource.setName("Nikhil");
	    db.insert(Constants.ROI_METHOD_TABLE_NAME, null,
		    dataSource.roiMethodToContentValues());
	  db.close();
    }
    ...
    private void selectAllSmall()
    {
	  db = helper.getReadableDatabase();
			Cursor c = db.query(Constants.ROI_METHOD_TABLE_NAME, null,
	...
	db = helper.getWritableDatabase();
	db.delete(Constants.ROI_METHOD_TABLE_NAME, null, null);
	db.close();
    }
	...
	db.update(Constants.ROI_METHOD_TABLE_NAME,
		dataSource.roiMethodToContentValues(), "_id = ?",
		new String[] { "1" });
	...
	db = helper.getWritableDatabase();
	db.delete(Constants.ROI_METHOD_TABLE_NAME, "_id = ?", new String[] { "7" });
	db.close();
	...
	db = helper.getWritableDatabase();
	db.delete(Constants.ROI_METHOD_TABLE_NAME, "_id = ?", new String[] { "34" });
	db.close();
