...
public class YieldMethodHelper implements IDBHelper {

    private final YieldMethodDataSource dataSource = YieldMethodDataSource
	    .getYieldMethodDataSource();
    private DBHelper dbHelper;

    private final List<YieldMethodDataSource> yieldMethodList = new ArrayList<YieldMethodDataSource>();
	...
	protected String doInBackground(String... operation) {
		...
		database.insert(Constants.YIELD_METHOD_TABLE_NAME, null,
		...
		onCreate();
		final SQLiteDatabase database = dbHelper.getWritableDatabase();
		database.update(Constants.YIELD_METHOD_TABLE_NAME,
			dataSource.yieldMethodToContentValues(),
		...
		onCreate();
		final SQLiteDatabase dataBase = dbHelper.getWritableDatabase();
		dataBase.delete(Constants.YIELD_METHOD_TABLE_NAME,
			dataSource.getWhereClause(), dataSource.getWhereArgs());
		...
		if (cursor.moveToFirst()) {
		    ...
		    do {
			yieldMethodList.add(dataSource
				.cursorToYieldMethod(cursor));
		    } while (cursor.moveToNext());

		}

		dataSource.setYieldMethodList(yieldMethodList);
