...
import com.energyeye.salesperson.properties.ElectricalCompanyDataSource;

public class ElectricalCompanyHelper implements IDBHelper {

    private final ElectricalCompanyDataSource dataSource = ElectricalCompanyDataSource
	    .getElectricalCompanyDataSource();
    private DBHelper dbHelper;

    private final List<ElectricalCompanyDataSource> electricalList = new ArrayList<ElectricalCompanyDataSource>();
	...
	dbHelper = new DBHelper(dataSource.getContext(), 1, Constants.DB_NAME,
		Constants.ELECTRICAL_COMPANY_CREATE_QUERY);
	return true;
	...
	protected String doInBackground(String... operation) {
		...
		database.insert(Constants.ELECTRICAL_COMPANY_TABLE_NAME, null,
			dataSource.ElectricalCompanyToContentValues());

	    } else if (operation[0].equalsIgnoreCase("update")) {

		onCreate();
		final SQLiteDatabase database = dbHelper.getWritableDatabase();
		database.update(Constants.ELECTRICAL_COMPANY_TABLE_NAME,
			dataSource.ElectricalCompanyToContentValues(),
		...
		onCreate();
		final SQLiteDatabase dataBase = dbHelper.getWritableDatabase();
		dataBase.delete(Constants.ELECTRICAL_COMPANY_TABLE_NAME,
			dataSource.getWhereClause(), dataSource.getWhereArgs());
		...
		if (cursor.moveToFirst()) {
		    ...
		    do {
			electricalList.add(dataSource
				.cursorToElectricalCompany(cursor));
		    } while (cursor.moveToNext());

		}

		dataSource.setElectricalCompanyList(electricalList);
